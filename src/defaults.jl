...
function default_settings!(m::AbstractModel)
   ...
   settings[:optimization_ftol] = Setting(:optimization_ftol, 1e-10,
    ...
    # Alternative policy
    baseline_policy = AltPolicy(:historical, eqcond, solve, color = RGB(0.0, 0.0, 0.0))
    settings[:alternative_policy] = Setting(:alternative_policy, baseline_policy, false, "apol", "Alternative monetary policy rule")
