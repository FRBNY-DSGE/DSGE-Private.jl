...
function load_data(m::AbstractDSGEModel; cond_type::Symbol = :none, try_disk::Bool = true,
    ...
    if recreate_data
        ...
        if any(occursin.("obs_exp_nominalrate", string.(vcat(cond_semi_names(m), cond_full_names(m)))))
            ...
            df[date_mainsample_end(m) .< df[!, :date] .<= date_conditional_end(m), [Symbol("obs_exp_nominalrate$i") for i in expected_ffr(m)]] .= Matrix{Union{Float64, Missing}}(spd_data_want)
        end
