        ...
        for class in classes
            ...
            output_files[output_var] =
                get_decomp_filename(m_new, m_old, input_type, cond_new, cond_old,
                                    product, class, pathfcn = rawpath, fileformat = :h5, forecast_string_new = forecast_string_new, forecast_string_old = forecast_string_old)
        end
    end
    return output_files
end
...
function get_decomp_mean_file(m_new::M, m_old::M, input_type::Symbol,
    ...
    get_decomp_filename(m_new, m_old, input_type, cond_new, cond_old, :decomp, class,
                        pathfcn = workpath, fileformat = :jld, forecast_string_new = forecast_string_new, forecast_string_old = forecast_string_old)
end
...
function write_forecast_decomposition(m_new::M, m_old::M, input_type::Symbol,
                                      ...
                                      verbose::Symbol = :low, forecast_string_new = "", forecast_string_old = "") where M<:AbstractModel
    for comp in [:data, :news, :shockdec, :dettrend, :para, :total]
        for class in classes
