...
Initializes subspec 2 of `GHLS`. The bounds for beta-distributed parameters have been changed from
...
Initializes subspec 3 of `GHLS`. Fix ρ_g.
"""
function ss3!(m::GHLS)
...
"""
```
ss4!(m::GHLS, divisor::Int)`
```

Initializes a GHLS model with measurement error equal to standard deviation of the
states scaled by a factor 1/divisor.
"""
function ss4!(m::GHLS, divisor::Int)
    scale = (1/divisor)
...
Initializes subspec 5 of `GHLS`. Change Lmean to be Normal(0, 2), since it seems
...
Initializes subspec 6 of `GHLS`. 'Diffuse prior' a la Herbst and Schorfheide 2014 (even though SW implementations are different, still want to use diffuse prior on our version)
