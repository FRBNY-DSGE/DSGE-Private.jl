    ...
    if !isnan(m[:βstar].value) && get_setting(m, :estimate_only_non_steady_state_parameters)
        ...
        if get_setting(m, :steady_state_only)
            ...
            if get_setting(m, :calibrate_income_targets)
                ...
                m[:sH_over_sL], _, _, _ = best_fit(m[:pLH].value, m[:pHL].value, m[:zlo].value,
...
"""
function best_fit(pLH::S, pHL::S, zlo::S, target::Vector{S}, lower::Vector{S}, upper::Vector{S},
                  us::Matrix{S}, zs::Matrix{S}, max_iter::Int = 20,
                  initial_guess::Vector{S} = [6.3, 0.03]) where {S<:AbstractFloat}

    skill_moments_f(x) = loss(collect(skill_moments(x, zlo, pLH, pHL, us, zs)), target)
    zlo_argmin = zlo

    res = optimize(skill_moments_f, lower[1], upper[1], iterations = max_iter) #Optim.Options(f_calls_limit = max_iter)) #, #=, [initial_guess[1]], Newton(),=# #Fminbox(NelderMead()),
