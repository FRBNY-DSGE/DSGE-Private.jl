...
function init_parameters!(m::HetDSGEGovDebt; testing_gamma::Bool = false)
    ...
    if subspec(m) in ["ss15", "ss16"]
        m <= parameter(:σ_e, 0.5, (1e-8, 1.), (1e-8, 1.), ModelConstructors.Exponential(),
                       RootInverseGamma(2., 0.1), fixed = false, tex_label = "\\sigma_e",
    ...
    na = get_setting(m, :na1_state) + get_setting(m, :na2_state)
    ns = get_setting(m, :ns)

    # Steady state grids for functional/distributional variables and market-clearing discount rate
    m <= SteadyStateParameterGrid(:lstar, fill(NaN, na*ns),
...
function setup_indices!(m::HetDSGEGovDebt)
    ...
    endo[:k′_t]   = nans_state+1:nans_state+1   # capital (NOT steadystate object K)
    endo[:i′_t1]  = nans_state+2:nans_state+2   # lagged nominal interest rate
    endo[:y′_t1]  = nans_state+3:nans_state+3   # lagged gdp
    endo[:w′_t1]  = nans_state+4:nans_state+4   # lagged real wages
    endo[:I′_t1]  = nans_state+5:nans_state+5   # lagged investment (NOT nominal interest rate, i)
    endo[:bg′_t]  = nans_state+6:nans_state+6   # govt debt

    # Exogenous scalar-valued states:
    endo[:b′_t]   = nans_state+7:nans_state+7   # discount factor shock
    endo[:g′_t]   = nans_state+8:nans_state+8   # govt spending
    endo[:z′_t]   = nans_state+9:nans_state+9   # tfp growth
    endo[:μ′_t]   = nans_state+10:nans_state+10 # investment shock
    endo[:λ_w′_t] = nans_state+11:nans_state+11 # wage markup
    endo[:λ_f′_t] = nans_state+12:nans_state+12 # price markup
    endo[:rm′_t]  = nans_state+13:nans_state+13 # monetary policy shock
    endo[:π_star′_t]  = nans_state+14:nans_state+14 # flexible income target

    # Function-valued jumps
    endo[:l′_t]   = nans_state+15:nans_state+nans_jump+14 # ell function
    nans_state_jump_15 = nans_state + nans_jump + 15

    # Scalar-valued jumps
    endo[:C′_t]         = nans_state_jump_15:nans_state_jump_15       # real interest rate
    endo[:R′_t]         = nans_state_jump_15+1:nans_state_jump_15+1   # real interest rate
    endo[:i′_t]         = nans_state_jump_15+2:nans_state_jump_15+2   # nominal interest rate
    endo[:t′_t]         = nans_state_jump_15+3:nans_state_jump_15+3   # transfers + dividends
    endo[:w′_t]         = nans_state_jump_15+4:nans_state_jump_15+4   # real wage
    endo[:L′_t]         = nans_state_jump_15+5:nans_state_jump_15+5   # hours worked
    endo[:π′_t]         = nans_state_jump_15+6:nans_state_jump_15+6   # inflation
    endo[:π_w′_t]       = nans_state_jump_15+7:nans_state_jump_15+7   # nominal wage inflation
    endo[:margutil′_t]  = nans_state_jump_15+8:nans_state_jump_15+8   # avg marginal utility
    endo[:y′_t]         = nans_state_jump_15+9:nans_state_jump_15+9   # gdp
    endo[:I′_t]         = nans_state_jump_15+10:nans_state_jump_15+10 # investment
    endo[:mc′_t]        = nans_state_jump_15+11:nans_state_jump_15+11 # marginal cost - this is ζ in HetDSGEGovDebtₖd.pdf
    endo[:Q′_t]         = nans_state_jump_15+12:nans_state_jump_15+12 # Tobin's qfunction
    endo[:capreturn′_t] = nans_state_jump_15+13:nans_state_jump_15+13 # return on capital
    endo[:tg′_t]        = nans_state_jump_15+14:nans_state_jump_15+14
    ...
    eqconds[:eq_π_star]  = nans_state_mul_2+29:nans_state_mul_2+29 # flexible inflation target
...
function init_states_and_jumps!(m::AbstractModel, states::Vector{Symbol},
    ...
    nans_state = (get_setting(m, :na1_state) + get_setting(m, :na2_state))* get_setting(m, :ns)
    nans_jump  = (get_setting(m, :na1_jump)  + get_setting(m, :na2_jump)) * get_setting(m, :ns)
    na1_state  = get_setting(m, :na1_state)
    na2_state  = get_setting(m, :na2_state)
    na1_jump   = get_setting(m, :na1_jump)
    na2_jump   = get_setting(m, :na2_jump)
    ...
    n_backward_looking_scalar_vars = get_setting(m, :nascalars)

    m <= Setting(:n_backward_looking_states, (na1_state + na2_state) *
    ...
    n_jump_scalar_vars = get_setting(m, :nyscalars) #n_jump_vars - (na1+na2)*n_jump_function_valued_vars

    m <= Setting(:n_jumps, (na1_jump+na2_jump)*n_jump_function_valued_vars +
