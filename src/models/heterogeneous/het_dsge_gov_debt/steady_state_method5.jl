function method5_steadystate!(m::HetDSGEGovDebt;
    ...
    if !isnan(m[:βstar].value) && get_setting(m, :estimate_only_non_steady_state_parameters)
        ...
        for ahi_guess in ahi_guesses
            try
                ...
                method5_find_steadystate!(m, na, ns, ne,
                                          egrid, ewts, g_of_e,
                                          f, agrid, sgrid,
                ...
                if doplots
                    ...
                    savefig(p, "method5_agrid_vs_D_beta=$(string(round(m[:βstar].value, digits = 3))).pdf")

                    p = plot(agrid, m[:cstar].value[1:na], label = "low skill")
                    plot!(p, agrid, m[:cstar].value[(na + 1):end], label = "high skill")
                    savefig(p, "method5_agrid_vs_C_beta=$(string(round(m[:βstar].value, digits = 3))).pdf")

                    p = plot(agrid, agrid - m[:cstar].value[1:na], label = "low skill")
                    plot!(p, agrid, agrid - m[:cstar].value[(na + 1):end], label = "high skill")
                    savefig(p, "method5_agrid_vs_Saving_beta=$(string(round(m[:βstar].value, digits = 3))).pdf")
                end
...
function method5_find_steadystate!(m::HetDSGEGovDebt, na::Int, ns::Int, ne::Int,
    ...
    elseif !isnan(m[:βstar].value)
        ...
        c_pol_out, c_as, ell, bp, KF, reject = method5_policy_hetdsgegovdebt(na, ns, ne, na_c, βlo_temp, R, ω, H, η, T, γ,
    ...
    if isnothing(roots_algorithm)
        while abs(excess) > tol && counter <= maxit # clearing markets
            β = (βlo + βhi) / 2.0
            c_pol_out, c_as, ell, bp, KF, reject = method5_policy_hetdsgegovdebt(na, ns, ne, na_c, β, R, ω, H, η, T, γ,
                                                                            m[:ehi].value, m[:elo].value,
                                                                            agrid, sgrid, c_pol_in, KF_in,
                                                                            f, egrid, ewts, g_of_e, transition_mat, qfunc,
                                                                            maxit = get_setting(m, :policy_maxit),
    ...
    elseif isa(roots_algorithm, AbstractBracketing)
        ...
        c_pol_in, c_as, ell, bp, KF, reject = method5_policy_hetdsgegovdebt(na, ns, ne, na_c, β, R, ω, H, η, T, γ,
                                                                       m[:ehi].value, m[:elo].value,
                                                                       agrid, sgrid, c_pol_in, KF_in,
                                                                       f, egrid, ewts, g_of_e, transition_mat, qfunc,
                                                                       maxit = get_setting(m, :policy_maxit),
    ...
    if doplots
        for is in 1:ns
            ...
            savefig(p, "method5_bgrid_vs_C_varye_s=$(sgrid[is]).pdf")
        end
    end
...
function bisect_β(β::S, na::Int, ns::Int, ne::Int, na_c::Int, R::S, ω::S, H::S, η::S, T::S, γ::S,
    ...
    ~, ~, bp, KF, ~ = method5_policy_hetdsgegovdebt(na, ns, ne, na_c, β, R, ω, H, η, T, γ,
...
function method5_policy_hetdsgegovdebt(na::Int, ns::Int, ne::Int, na_c::Int, β::S, R::S, ω::S, H::S, η::S,
    ...
    @inline myq_fn(x::S0) where {S0 <: Real} = mollifier_hetdsgegovdebt(x,  ehi, elo)
    lr = .3
    ell_pol_in = similar(c_as)
    method5_euler_equation!(ell_pol_in, β, 1., R, γ, T, ω, H, c_as, c_as, na, ns, (agrid[end] - agrid[1]) / na,
                    agrid, sgrid, f, myq_fn)
    ell_pol_out = similar(ell_pol_in)
    while dist > euler_tol && counter <= maxit
        method5_euler_equation!(ell_pol_out, β, 1., R, γ, T, ω, H, c_as, c_as, na, ns, (agrid[end] - agrid[1]) / na,
                        agrid, sgrid, f, myq_fn)

        ell_pol_out .= lr .* ell_pol_out + (1 - lr) .* ell_pol_in
    ...
    return c_pol, vec(c_as), vec(ell_pol_out), vec(bp), D_as, reject
end

@inline function method5_euler_equation!(ret_ell::AbstractMatrix{S0}, β::S1, b_t::S0,
    ...
    @inbounds @simd for iss = 1:ns
        ...
        @inbounds @simd for ia = 1:na
            ...
            ret_ell[ia, iss] = (ι * β * b_t * R_t) * ell_euler
        end
    end
