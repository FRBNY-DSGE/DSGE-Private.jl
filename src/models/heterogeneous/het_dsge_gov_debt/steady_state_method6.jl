function method6_steadystate!(m::HetDSGEGovDebt;
    ...
    if !isnan(m[:βstar].value) && get_setting(m, :estimate_only_non_steady_state_parameters)
        ...
        for ahi_guess in ahi_guesses
            try
                ...
                method6_find_steadystate!(m, na, ns, ne,
                                          egrid, ewts, g_of_e,
                                          f, agrid, sgrid,
                ...
                if doplots
                    ...
                    savefig(p, "method6_agrid_vs_D_beta=$(string(round(m[:βstar].value, digits = 3))).pdf")

                    p = plot(agrid, m[:cstar].value[1:na], label = "low skill")
                    plot!(p, agrid, m[:cstar].value[(na + 1):end], label = "high skill")
                    savefig(p, "method6_agrid_vs_C_beta=$(string(round(m[:βstar].value, digits = 3))).pdf")

                    p = plot(agrid, agrid - m[:cstar].value[1:na], label = "low skill")
                    plot!(p, agrid, agrid - m[:cstar].value[(na + 1):end], label = "high skill")
                    savefig(p, "method6_agrid_vs_Saving_beta=$(string(round(m[:βstar].value, digits = 3))).pdf")
                end
...
function method6_find_steadystate!(m::HetDSGEGovDebt, na::Int, ns::Int, ne::Int,
    ...
    c_pol_in       = (R - 1) * repeat(agrid, 1, ns) .+ ω * H * repeat(sgrid', na, 1)
    ...
    elseif !isnan(m[:βstar].value)
        ...
        c_pol_out, bp, KF, reject = method6_policy_hetdsgegovdebt(na, ns, ne, na_c, βlo_temp, R, ω, H, η, T, γ,
    ...
    if isnothing(roots_algorithm)
        while abs(excess) > tol && counter <= maxit # clearing markets
            β = (βlo + βhi) / 2.0
            c_pol_out, bp, KF, reject = method6_policy_hetdsgegovdebt(na, ns, ne, na_c, β, R, ω, H, η, T, γ,
                                                                      m[:ehi].value, m[:elo].value,
                                                                      agrid, sgrid, c_pol_in, KF_in,
                                                                      f, egrid, ewts, g_of_e, transition_mat, qfunc,
                                                                      maxit = get_setting(m, :policy_maxit),
    ...
    elseif isa(roots_algorithm, AbstractBracketing)
        ...
        c_pol_in, c_as, bp, KF, reject = method6_policy_hetdsgegovdebt(na, ns, ne, na_c, β, R, ω, H, η, T, γ,
                                                                       m[:ehi].value, m[:elo].value,
                                                                       agrid, sgrid, c_pol_in, KF_in,
                                                                       f, egrid, ewts, g_of_e, transition_mat, qfunc,
                                                                       maxit = get_setting(m, :policy_maxit),
...
function bisect_β(β::S, na::Int, ns::Int, ne::Int, na_c::Int, R::S, ω::S, H::S, η::S, T::S, γ::S,
    ...
    ~, ~, bp, KF, ~ = method6_policy_hetdsgegovdebt(na, ns, ne, na_c, β, R, ω, H, η, T, γ,
...
function method6_policy_hetdsgegovdebt(na::Int, ns::Int, ne::Int, na_c::Int, β::S, R::S, ω::S, H::S, η::S,
                                       T::S, γ::S, ehi::S, elo::S, agrid::Vector{S},
                                       sgrid::Vector{S}, c_pol_in::AbstractArray{S, 2}, KF_in::AbstractArray{S, 2},
    ...
    # Interpolate c(a, s) -> c(b, s, e)
    c_pol = cas2cbse(c_pol_in, agrid, sgrid, egrid, bgrid, na, ns, ne, γ, ω, H, T, R)
    ...
    return c_as, vec(bp), D_as, reject
end
