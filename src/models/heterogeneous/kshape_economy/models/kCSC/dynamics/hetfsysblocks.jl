## ============================================================================
## Heterogeneous-agent block for kCSC
##
## Include this file inside the kCSC Fsys preamble. It assumes these names are
## already in scope:
##   F, Xt, Xt1, Yt, Yt1, θ, state_id, control_id, StateSS, ControlSS,
##   Γ_state, Γ_ctrl, DC, IDC, DCD, IDCD, dct_compression_indices,
##   nb, na, nse, ns, ns_1, ns_2, nCOP, oc, nRedMarg, nFullMarg, grids, m
##
## The source economics are ported from F_sys_CSC_ref_tvcopula.jl. The adapter
## below replaces the old function arguments with objects from the Fsys scope.
## ============================================================================
...
State = Xt1
Stateminus = Xt
Controlnext_sparse = Yt1
Control_sparse = Yt
Gamma_state = Γ_state
Gamma_control = Γ_ctrl

SS_stats = get(m.dicts, :SS_stats, Dict{String, Any}())

function _kCSC_θget(θ, key::Symbol, default = nothing)
    return haskey(θ, key) ? θ[key] : default
end

param = Dict{String, Any}()
if haskey(m.dicts, :param)
    merge!(param, m.dicts[:param])
end

function _kCSC_set_param!(param::Dict{String, Any}, θ, key::String, sym::Symbol; default = nothing)
    val = _kCSC_θget(θ, sym, default)
    if val !== nothing
        param[key] = val
    end
    return nothing
...
for (key, sym) in [
    "sigma" => :σ, "xi" => :ξ, "sigma_chi" => :σ_χ, "mu_chi" => :μ_χ,
    "nu" => :ν, "beta" => :β, "death_rate" => :dr, "GHH" => :GHH,
    "b_aux" => :b_aux, "rhoS" => :ρ_S, "sigmaS" => :σ_S, "in" => :in,
    "out" => :out, "Eshare" => :Eshare, "in_s" => :in_s, "out_s" => :out_s,
    "in_l" => :in_l, "out_l" => :out_l, "eta" => :η, "pi_bar" => :π_bar,
    "Eratio" => :Eratio, "fix_ratio" => :fix_ratio, "fix" => :fix,
    "b_share" => :b_share, "zeta" => :ζ, "rho" => :ρ_ces, "a" => :a_share,
    "w" => :w_share, "Z" => :Z_ss, "v" => :v_util, "delta_ss" => :δ_ss,
    "u_1" => :u_1, "v_f_1" => :v_f_1, "f_1" => :f_1, "iota_1" => :ι_1,
    "lambda_1" => :λ_1, "fix_L_1" => :fix_L_1, "u_2" => :u_2,
    "v_f_2" => :v_f_2, "f_2" => :f_2, "iota_2" => :ι_2,
    "lambda_2" => :λ_2, "fix_L_2" => :fix_L_2, "earning_ratio" => :earning_ratio,
    "tau_w" => :τ_w, "tau_a" => :τ_a, "tau_b" => :τ_b, "b_ratio" => :b_ratio,
    "LT" => :LT, "B_gov_ratio" => :B_gov_ratio, "R_cb" => :R_cb_ss,
    "pi_cb" => :π_cb, "Rprem" => :Rprem, "b_min" => :b_min,
    "A_g_ratio" => :A_g_ratio, "theta_b" => :θ_b, "THETA" => :Θ,
    "Ab_ratio" => :Ab_ratio, "b_a_aux" => :b_a_aux, "b_a_aux2" => :b_a_aux2,
    "beta_b" => :β_b, "B_MMMF_ratio" => :B_MMMF_ratio, "q" => :q_ss,
    "tau_cp" => :τ_cp, "Lambda_bank" => :β_L, "frac_b" => :frac_b,
    "fix2" => :fix_ratio, "guessadj" => :guessadj, "n_1" => :n_1, "n_2" => :n_2,
    "w_bar" => :w_bar, "w_bar_1" => :w_bar_1, "w_bar_2" => :w_bar_2,
    "Lambda_aux" => :λ_aux, "Lambda_b_aux" => :λ_b_aux, "lambda_hh_aux" => :λ_b_aux,
    "DELTA" => :δ_1, "delta_0" => :δ_ss, "delta_1" => :δ_1,
    "phi" => :ϕ, "phi_B" => :ϕ_b, "phi_pi" => :ϕ_π, "phi_u" => :ϕ_u,
    "phi_pi_QE" => :ϕ_π_QE, "phi_u_QE" => :ϕ_u_QE, "kappa" => :κ,
    "rho_R" => :ρ_R, "rho_G" => :ρ_G, "rho_D" => :ρ_D, "rho_eta" => :ρ_η,
    "rho_iota" => :ρ_ι, "rho_BB" => :ρ_BB, "rho_B_F" => :ρ_B_F,
    "rho_MP" => :ρ_mp, "rho_PSI_RP" => :ρ_ψ_rp, "rho_PSI_W" => :ρ_ψ_w,
    "rho_X_QE" => :ρ_X_QE, "rho_passive_QE" => :ρ_passive_QE,
    "rho_Z" => :ρ_Z, "rho_B" => :ρ_B, "rho_w" => :ρ_w,
    "rho_ZZ_1" => :ρ_ZZ_1, "rho_ZZ_2" => :ρ_ZZ_2,
    "rho_ZZ_3" => :ρ_ZZ_3, "rho_ZZ_4" => :ρ_ZZ_4,
    "rho_w_1" => :ρ_w_1, "rho_w_2" => :ρ_w_2,
    "gamma_pi" => :γ_π, "gamma_T" => :γ_T, "GAMMA" => :GAMMA,
    "MMF_ratio_1" => :MMF_ratio_1, "d" => :d, "d_1" => :d_1, "d_2" => :d_2,
    "alpha_1" => :alpha_1, "alpha_2" => :alpha_2,
    "psi_1" => :psi_1, "psi_2" => :psi_2, "omega" => :ω, "sigma2" => :σ
]
    _kCSC_set_param!(param, θ, key, sym)
end

param["beta_aux"] = param["beta"]

for matrix_key in (:P_SS, :P_SS2, :P_SS3, :P_SS4)
    if haskey(grids, matrix_key)
        param[String(matrix_key)] = grids[matrix_key]
    elseif haskey(m.grids, matrix_key)
        param[String(matrix_key)] = m.grids[matrix_key]
    end
end

grid = Dict{String, Any}()
if haskey(m.dicts, :grid)
    merge!(grid, m.dicts[:grid])
end
for (k, v) in grids
    grid[String(k)] = v
end
for (key, val) in [
    "nb" => nb, "na" => na, "nse" => nse, "ns" => ns, "ns_1" => ns_1, "ns_2" => ns_2,
    "nCOP" => nCOP, "oc" => oc, "numstates" => get_setting(m, :n_backward_looking_states),
    "nFullMarg" => nFullMarg, "nRedMarg" => nRedMarg
]
    grid[key] = val
end
for setting_key in (:nb_copula, :na_copula, :nse_copula, :compressionIndexesCOP)
    try
        grid[String(setting_key)] = get_setting(m, setting_key)
    catch
    end
end

if !isdefined(@__MODULE__, :InvGamma)
    InvGamma = try
        get_setting(m, :InvGamma)
    catch
        sparse(I, length(StateSS) + length(ControlSS), length(StateSS) + length(Yt))
    end
end
## Initializations
...
NW = param["xi"] / (1 + param["xi"]) * param["GHH"] * n_2 * W_2 .* ones(nb, na, nse)
NW[:, :, 1:ns_1]                          .= param["xi"] / (1 + param["xi"]) * param["GHH"] * n_1 * W_1
NW[:, :, ns+1:ns+ns_1]                    .= param["xi"] / (1 + param["xi"]) * param["GHH"] * 1 * param["w_bar_1"]
NW[:, :, ns+ns_1+1:ns+ns_1+ns_2]          .= param["xi"] / (1 + param["xi"]) * param["GHH"] * 1 * param["w_bar_2"]

WW           = (1 - tau_w) .* (NW .* auxWW1 .+ NW .* auxWW2)
WW[:, :, end] = (1 - tau_w) .* (param["Eratio"] * PROFIT) / Eshare .* ones(nb, na)
...
RHS[nx+C_ind]    = q_cons(c_a_star, c_n_star, param["w_bar_1"], param["w_bar_2"],
                          n_1, n_2, tau_w, AProb, MU_tilde, meshes, param, grid)
...
RHS[nx+UB_ind] = param["xi"]/(1+param["xi"]) * param["GHH"] *
                 (grid["se"][ns+1:ns+ns_1]'   * MU_tilde_se[ns+1:ns+ns_1])   * n_1 * W_1 +
                 param["xi"]/(1+param["xi"]) * param["GHH"] *
                 (grid["se"][ns+ns_1+1:ns+ns_1+ns_2]' * MU_tilde_se[ns+ns_1+1:ns+ns_1+ns_2]) * n_2 * W_2
...
Lincome = L_1*W_1 + L_2*W_2
RHS[nx+T_ind] = tau_w*(Lincome + UB) + tau_a*PROFIT
...
RHS[nx+V_1_ind] = M_1/param["iota_1"] * (J[1:ns_1]' * grid["se_dist"][1:ns_1]) /
                  sum(grid["se_dist"][1:ns_1])
RHS[nx+V_2_ind] = M_2/param["iota_2"] * (J[ns_1+1:ns_1+ns_2]' * grid["se_dist"][ns_1+1:ns_1+ns_2]) /
                  sum(grid["se_dist"][ns_1+1:ns_1+ns_2])
...
if param["GHH"] == 1
    RHS[nx+n_1_ind] = ((1-tau_w)*W_1/param["psi_1"])^(1/param["xi"])
    RHS[nx+n_2_ind] = ((1-tau_w)*W_2/param["psi_2"])^(1/param["xi"])
else
    RHS[nx+n_1_ind] = 1.0
    RHS[nx+n_2_ind] = 1.0
end
...
RHS[nx+I_ind]         = iota*(1 + param["phi"]/2*(log(x_k))^2) *
                         ...
                         (1 - param["delta_0"]*v^param["delta_1"])*K
...
for (eq_name, eq_idx) in eqconds
    F[eq_name] = Difference[eq_idx]
end
...
hetfsys_outputs = (
    ...
    AC = AC,
    AProb = AProb,
    P_transition_exp = P_transition_exp,
    P_transition_dist = P_transition_dist,
)
