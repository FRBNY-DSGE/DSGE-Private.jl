...
function Cal_SS_stats(mu_dist::Array{Float64,3}, c_a_guess::Array{Float64,3},
    ...
    WW_pretax = fill(coeff * n_1 * param["w_bar_1"], nb, na, nse)
    WW_pretax[:, :, (ns_1 + 1):ns] .= coeff * n_2 * param["w_bar_2"]
    WW_pretax[:, :, (ns + ns_1 + 1):(ns + ns)] .= coeff * n_2 * param["w_bar_2"]
    ...
    SS_stats["n_1"] = param["n_1"]
    SS_stats["n_2"] = param["n_2"]
    SS_stats["r_k"] = r_k
    SS_stats["mc"] = mc
    ...
    SS_stats["tau_L"] = param["tau_L"]
    SS_stats["tau_P"] = param["tau_P"]
    SS_stats["Income_Tax"] = get(param, "Income_Tax", TAX_revenue - param["tau_a"] * (1 - param["Eratio"]) * Profit)
    ...
    X_a_RHS = WW .* meshes["se"].^(1 - tau_P) .+ (param["q"] .+ RR_aux) .* meshes["a"] .+ RBRB_aux .* meshes["b"] .-
              param["q"] .* a_a_star .- b_a_star .+ SS_stats["LT"]
    X_n_RHS = WW .* meshes["se"].^(1 - tau_P) .+ (param["q"] .+ RR_aux) .* meshes["a"] .+ RBRB_aux .* meshes["b"] .-
    ...
    if param["GHH"] == 1
        C_agg = X_agg + sum((1 - tau_L) * (1 - tau_P) / (1 + 1 / xi) .* NW_aux[:, :, 1:ns].^(1 - tau_P) .* mu_dist_tilde[:, :, 1:ns])
        C_agg2 = X_agg2 + sum((1 - tau_L) * (1 - tau_P) / (1 + 1 / xi) .* NW_aux[:, :, 1:ns].^(1 - tau_P) .* mu_dist_tilde[:, :, 1:ns])
    else
        C_agg = X_agg
        C_agg2 = X_agg2
    end
    ...
    WW_Gini = (1 - tau_L) .* NW_Gini.^(1 - tau_P)
    WW_Gini[:, :, end] .= (1 - tau_L) * ((param["Eratio"] * SS_stats["Profit"]) / param["Eshare"])^(1 - tau_P)

    income_unsorted = WW_Gini .* meshes["se_aux"].^(1 - tau_P) .+ SS_stats["r_a_aux"] .* meshes["a"] .+
    ...
    labor_income_unsorted = WW_Gini .* meshes["se_aux"].^(1 - tau_P)
