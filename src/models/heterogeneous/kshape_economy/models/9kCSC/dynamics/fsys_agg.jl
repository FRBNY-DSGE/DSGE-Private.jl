...
function Fsys_agg(F, m, grid, StateSS, ControlSS, Xt1, Yt1, Xt, Yt, state_id, control_id, reg=1)
    ...
    #===================================================================#
    # ── CONTROL EQUATIONS ──────────────────────────────────────────────
    # Summary controls filled by the heterogeneous household block.
    # Set to zero here; the Jacobian assembles them analytically.
    #===================================================================#
    F[:MRS_ind]       = 0.
    F[:A_hh_ind]      = 0.
    F[:B_hh_ind]      = 0.
    ...
    #===================================================================#
    #control eq14: capital utilisation v (identity: v_t = v_{t+1})
    ...
    if m[:GHH] == 1
        F[:eq_control_n_1] = n_1_t - ((1 - m[:τ_w]) * w_1′_t / m[:psi_1])^(1 / m[:ξ])
    else
    ...
    if m[:GHH] == 1
        F[:eq_control_n_2] = n_2_t - ((1 - m[:τ_w]) * w_2′_t / m[:psi_2])^(1 / m[:ξ])
    else
