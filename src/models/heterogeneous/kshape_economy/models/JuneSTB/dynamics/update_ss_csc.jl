...
"""Update derived steady-state quantities stored on a `kCSC9` model object."""
function update_ss_csc!(m::kCSC9)
    param = m.dicts[:param]
    param_update = Dict{String,Any}("iota" => param["iota_1"])
    update_ss_csc!(m.dicts[:SS_stats], param, param_update, m.dicts[:grid])
    return m
end
