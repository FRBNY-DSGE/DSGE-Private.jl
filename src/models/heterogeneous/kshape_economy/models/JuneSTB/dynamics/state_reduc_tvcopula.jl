...
function state_reduc_tvcopula!(param::Dict, grid::Dict, SS_stats::Dict,
    ...
    Yss = vcat(
        ...
        log(SS_stats["Income_Tax"]), log(SS_stats["J_bar_1"]), log(SS_stats["J_bar_2"]),
...
"""
    state_reduc_csc(m::kCSC9)

Model-object adapter for the JuneSTB steady-state/Jacobian pipeline.  The
underlying reducer uses the same dictionary-based interface as the sep-imf
steady-state code; this method stores its reduced objects on `m` for
`jacobian!` and downstream system construction.
"""
function state_reduc_csc(m::kCSC9)
    reduc = state_reduc_tvcopula!(m.dicts[:param], m.dicts[:grid],
                                  m.dicts[:SS_stats], m.grids[:mu_dist],
                                  m.grids[:Value], m.grids[:mutil_c],
                                  m.grids[:Va])

    m.dicts[:reduction] = reduc
    m.grids[:StateSS] = reduc.Xss
    m.grids[:ControlSS] = reduc.Yss
    m.dicts[:Xss] = reduc.Xss
    m.dicts[:Yss] = reduc.Yss
    m.dicts[:Gamma_state] = reduc.Gamma_state
    m.dicts[:Gamma_control] = reduc.Gamma_control
    m.dicts[:InvGamma] = reduc.InvGamma
    return reduc
end
