...
function compare_matrices(AD, FD, name::String, eqn_state::Bool, var_state;
                          atol=1e-4,
                          exceptions=Vector{Vector{Int}}(),
                          write_md::Bool=true,
                          mdfile::String="matrix_diffs.md",
                          append_md::Bool=true)
    ...
    @testset "$name" begin
        ...
        if size(AD) == size(FD)
            ...
            if !isempty(diffs)
                ...
                io = write_md ? open(mdfile, append_md ? "a" : "w") : nothing

                try
                    if write_md
                        println(io, "## $name")
                        println(io, "")
                        println(io, "$(length(diffs)) differing elements.")
                        println(io, "")
                    end

                    for i in diffs
                        r, c = Tuple(CartesianIndices(AD)[i])
                        eqn = eqn_state ? state_eqn_names[r] : control_eqn_names[r + n_hh_summary_julia]
                        var = var_state ? state_var_names[c] : control_var_names[c]
                        ...
                        if write_md
                            println(io, "### [$r, $c]")
                            println(io, "")
                            println(io, "- eqn (row): `$eqn`")
                            println(io, "- var (col): `$var`")
                            println(io, "- julia: `$(AD[i])`")
                            println(io, "- original: `$(FD[i])`")
                            println(io, "")
                            println(io, "---")
                            println(io, "")
                        end
