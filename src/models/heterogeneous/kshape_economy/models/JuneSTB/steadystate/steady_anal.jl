...
function _steady_anal_consumption_weight(c_a_guess, c_n_guess, AProb, meshes, param, grid, SS_stats)
    ...
    if param["GHH"] == 1
        ...
        cons_weight[:, :, 1:ns] .+= ((1 - param["tau_L"]) * (1 - param["tau_P"]) / (1 + 1 / param["xi"])) .*
                                     nw_aux[:, :, 1:ns].^(1 - param["tau_P"])
    end
...
function steady_anal(grid, param, meshes, SS_stats, AProb, mu_dist, n, H_tilde,
    ...
    # WW_aux = (1-param.tau_L)*NW_aux
    WW_aux = (1 - param["tau_L"]) .* NW_aux.^(1 - param["tau_P"])

    # WW_aux(:,:,end) = (1-param.tau_L)*(param.Eratio*SS_stats.Profit)/param.Eshare
    WW_aux[:, :, end] .= (1 - param["tau_L"]) * ((param["Eratio"] * SS_stats["Profit"]) / param["Eshare"])^(1 - param["tau_P"])
    ...
    total_income_mesh_aux = WW_aux .* meshes_se_aux.^(1 - param["tau_P"]) .+
    ...
    labor_income_mesh_aux = WW_aux .* meshes_se1_aux.^(1 - param["tau_P"])
    ubincome_mesh_aux = WW_aux .* meshes_se2_aux.^(1 - param["tau_P"])
    profit_income_mesh_aux = WW_aux .* meshes_se3_aux.^(1 - param["tau_P"])
    ...
    # Income distribution
    income_unsorted = WW_aux .* meshes_se_aux.^(1 - param["tau_P"]) .+
    SS_stats["r_a_aux"] .* meshes["a"] .+
    bond_return_pos .* meshes["b"] .* (meshes["b"] .>= 0) .+
    bond_return_neg .* meshes["b"] .* (meshes["b"] .< 0) .+
    inc_transfer
    ...
    tau_L = param["tau_L"]
    income_comp_all = [total_labor_all/(1-tau_L); total_profit_income_all/(1-tau_L); total_asset_income_all; total_other_income_all] ./
                      (total_labor_all/(1-tau_L) + total_profit_income_all/(1-tau_L) + total_asset_income_all + total_other_income_all)
    income_comp_B60 = [total_labor_B60/(1-tau_L); total_profit_income_B60/(1-tau_L); total_asset_income_B60; total_other_income_B60] ./
                      (total_labor_B60/(1-tau_L) + total_profit_income_B60/(1-tau_L) + total_asset_income_B60 + total_other_income_B60)
    income_comp_Q1 = [total_labor_Q1/(1-tau_L); total_profit_income_Q1/(1-tau_L); total_asset_income_Q1; total_other_income_Q1] ./
                     (total_labor_Q1/(1-tau_L) + total_profit_income_Q1/(1-tau_L) + total_asset_income_Q1 + total_other_income_Q1)
    income_comp_Q2 = [total_labor_Q2/(1-tau_L); total_profit_income_Q2/(1-tau_L); total_asset_income_Q2; total_other_income_Q2] ./
                     (total_labor_Q2/(1-tau_L) + total_profit_income_Q2/(1-tau_L) + total_asset_income_Q2 + total_other_income_Q2)
    income_comp_Q3 = [total_labor_Q3/(1-tau_L); total_profit_income_Q3/(1-tau_L); total_asset_income_Q3; total_other_income_Q3] ./
                     (total_labor_Q3/(1-tau_L) + total_profit_income_Q3/(1-tau_L) + total_asset_income_Q3 + total_other_income_Q3)
    income_comp_Q4 = [total_labor_Q4/(1-tau_L); total_profit_income_Q4/(1-tau_L); total_asset_income_Q4; total_other_income_Q4] ./
                     (total_labor_Q4/(1-tau_L) + total_profit_income_Q4/(1-tau_L) + total_asset_income_Q4 + total_other_income_Q4)
    income_comp_Q5 = [total_labor_Q5/(1-tau_L); total_profit_income_Q5/(1-tau_L); total_asset_income_Q5; total_other_income_Q5] ./
                     (total_labor_Q5/(1-tau_L) + total_profit_income_Q5/(1-tau_L) + total_asset_income_Q5 + total_other_income_Q5)
    income_comp_P90 = [total_labor_P90/(1-tau_L); total_profit_income_P90/(1-tau_L); total_asset_income_P90; total_other_income_P90] ./
                      (total_labor_P90/(1-tau_L) + total_profit_income_P90/(1-tau_L) + total_asset_income_P90 + total_other_income_P90)
    income_comp_P99 = [total_labor_P99/(1-tau_L); total_profit_income_P99/(1-tau_L); total_asset_income_P99; total_other_income_P99] ./
                      (total_labor_P99/(1-tau_L) + total_profit_income_P99/(1-tau_L) + total_asset_income_P99 + total_other_income_P99)
    income_comp_P999 = [total_labor_P999/(1-tau_L); total_profit_income_P999/(1-tau_L); total_asset_income_P999; total_other_income_P999] ./
                       (total_labor_P999/(1-tau_L) + total_profit_income_P999/(1-tau_L) + total_asset_income_P999 + total_other_income_P999)

    # income_comp_2: like income_comp but separates equity from bonds
    income_comp_all_2 = [total_labor_all/(1-tau_L); total_profit_income_all/(1-tau_L); total_a_all; total_b_all; total_other_income_all] ./
                        (total_labor_all/(1-tau_L) + total_profit_income_all/(1-tau_L) + total_asset_income_all + total_other_income_all)
    income_comp_B60_2 = [total_labor_B60/(1-tau_L); total_profit_income_B60/(1-tau_L); total_a_B60; total_b_B60; total_other_income_B60] ./
                        (total_labor_B60/(1-tau_L) + total_profit_income_B60/(1-tau_L) + total_asset_income_B60 + total_other_income_B60)
    income_comp_Q1_2 = [total_labor_Q1/(1-tau_L); total_profit_income_Q1/(1-tau_L); total_a_Q1; total_b_Q1; total_other_income_Q1] ./
                       (total_labor_Q1/(1-tau_L) + total_profit_income_Q1/(1-tau_L) + total_asset_income_Q1 + total_other_income_Q1)
    income_comp_Q2_2 = [total_labor_Q2/(1-tau_L); total_profit_income_Q2/(1-tau_L); total_a_Q2; total_b_Q2; total_other_income_Q2] ./
                       (total_labor_Q2/(1-tau_L) + total_profit_income_Q2/(1-tau_L) + total_asset_income_Q2 + total_other_income_Q2)
    income_comp_Q3_2 = [total_labor_Q3/(1-tau_L); total_profit_income_Q3/(1-tau_L); total_a_Q3; total_b_Q3; total_other_income_Q3] ./
                       (total_labor_Q3/(1-tau_L) + total_profit_income_Q3/(1-tau_L) + total_asset_income_Q3 + total_other_income_Q3)
    income_comp_Q4_2 = [total_labor_Q4/(1-tau_L); total_profit_income_Q4/(1-tau_L); total_a_Q4; total_b_Q4; total_other_income_Q4] ./
                       (total_labor_Q4/(1-tau_L) + total_profit_income_Q4/(1-tau_L) + total_asset_income_Q4 + total_other_income_Q4)
    income_comp_Q5_2 = [total_labor_Q5/(1-tau_L); total_profit_income_Q5/(1-tau_L); total_a_Q5; total_b_Q5; total_other_income_Q5] ./
                       (total_labor_Q5/(1-tau_L) + total_profit_income_Q5/(1-tau_L) + total_asset_income_Q5 + total_other_income_Q5)
    income_comp_P90_2 = [total_labor_P90/(1-tau_L); total_profit_income_P90/(1-tau_L); total_a_P90; total_b_P90; total_other_income_P90] ./
                        (total_labor_P90/(1-tau_L) + total_profit_income_P90/(1-tau_L) + total_asset_income_P90 + total_other_income_P90)
    income_comp_P99_2 = [total_labor_P99/(1-tau_L); total_profit_income_P99/(1-tau_L); total_a_P99; total_b_P99; total_other_income_P99] ./
                        (total_labor_P99/(1-tau_L) + total_profit_income_P99/(1-tau_L) + total_asset_income_P99 + total_other_income_P99)
    income_comp_P999_2 = [total_labor_P999/(1-tau_L); total_profit_income_P999/(1-tau_L); total_a_P999; total_b_P999; total_other_income_P999] ./
                         (total_labor_P999/(1-tau_L) + total_profit_income_P999/(1-tau_L) + total_asset_income_P999 + total_other_income_P999)
    ...
    income_ss = (WW_aux .* meshes_se_aux.^(1 - param["tau_P"]) .+
                 SS_stats["r_a_aux"] .* meshes["a"] .+
    ...
    WW_se_mesh = WW_aux .* meshes_se_aux.^(1 - param["tau_P"])
    ...
    denom_se = gradient(log.(WW_se .* grid_se_aux.^(1 - param["tau_P"])))
#=
    for mm in 1:nb
        for kk in 1:na
