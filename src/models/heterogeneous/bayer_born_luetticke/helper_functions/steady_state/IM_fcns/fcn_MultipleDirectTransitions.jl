function MultipleDirectTransition!(m_a_star::AbstractArray{T,3},
    ...
    if parallel
        return _parallel_MultipleDirectTransition!(m_a_star, m_n_star, k_a_star, distr,
                                                   λ, Π, dims, grids, ϵ; iters = iters)
    end
...
function _parallel_MultipleDirectTransition!(m_a_star::AbstractArray{T,3},
                                             m_n_star::AbstractArray{T,3},
                                             k_a_star::AbstractArray{T,3},
                                             distr::AbstractArray{T,3},
                                             λ::T,
    ...
    nm,     nk,     ny     = dims
    m_grid, k_grid, y_grid = grids
    mky_CI                 = CartesianIndices((nm, nk, ny))
    ...
    while (dist>ϵ) && (count<iters)
        dPrime .= 0. # reset all entries to zero after each loop. This avoids making allocations, which is faster in the end
        @inbounds begin
            Threads.@threads for mmkkzz in mky_CI
                mm, kk, zz = mmkkzz[1], mmkkzz[2], mmkkzz[3] # loop over income (zz), illiquid asset (kk), and liquid asset (mm)
                dd=distr[mm,kk,zz]
                IDD_a = idm_a[mm,kk,zz] .+ (idk_a[mm,kk,zz]-1) .* nm
                IDD_n = idm_n[mm,kk,zz] .+ (kk-1) .* nm
                DLL_a = dd .* wL_k_a[mm,kk,zz] .* wL_m_a[mm,kk,zz]
