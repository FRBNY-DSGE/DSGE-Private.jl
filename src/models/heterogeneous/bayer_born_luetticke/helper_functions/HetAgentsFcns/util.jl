...
#---------------------------------------------------------------------------
_bbl_mutil(c::Array, ξ::Float64)      = 1.0 ./ ((c.*c).*(c.*c)) # c.^ξ, and ξ = 4.
_bbl_invmutil(mu::Array, ξ::Float64)  = 1.0 ./ (sqrt.(sqrt.(mu))) # mu.^(1.0./ξ), and ξ = 4.
_bbl_mutil(c::Array, ξ::AbstractParameter)      = _bbl_mutil(c, get_untransformed_values(ξ))
