...
function _original_construct_liquid_asset_grid_bbl(mmin::T, mmax::T, nm::Int) where {T <: Real}
    return exp.(range(0, stop = log(mmax - mmin + 1.), length = nm)) .+ mmin .- 1.
end
function _original_construct_illiquid_asset_grid_bbl(kmin::T, kmax::T, nk::Int) where {T <: Real}
    return exp.(range(log(kmin + 1.), stop = log(kmax + 1.), length = nk)) .- 1.
end

function original_init_grids!(m::BayerBornLuetticke{T}; coarse::Bool = false) where {T <: Real}
    ...
    # Liquid asset grid
    m_grid = _original_construct_liquid_asset_grid_bbl(mmin, mmax, nm)
    ...
    # Illiquid asset grid
    grids[:k_grid] = Grid(_original_construct_illiquid_asset_grid_bbl(kmin, kmax, nk),
