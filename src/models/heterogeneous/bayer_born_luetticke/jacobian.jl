...
#=
  DCD[1]  = mydctmx(n_copula)
    ...
    @show n_vars
    ## TO TEST FSYS FCT
    #println("testing evaluation of Fsys at 0,0")
    #println(obj_fnct(zeros(nxB+nxA)))
...
#=
    ...
    println(sum(diff_Fsys_test_bool))
    println("indices where nonzero value")
    ind_vals_non = findall(x -> x>0,diff_Fsys_test_bool)
    println(ind_vals_non)
    println("is nan")
