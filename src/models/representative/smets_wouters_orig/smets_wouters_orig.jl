...
function init_parameters!(m::SmetsWoutersOrig)
  ...
  if subspec(m) in ["ss21", "ss22", "ss25", "ss26", "ss28", "ss29", "ss41", "ss42"]
      m <= parameter(:ζ_p_r2, 0.7813, (1e-5, 0.999), (1e-5, 0.999), ModelConstructors.SquareRoot(), BetaAlt(0.5, 0.1), fixed=false,
                     description="ζ_p: The Calvo parameter. In every period, intermediate goods producers optimize prices with probability (1-ζ_p). With probability ζ_p, prices are adjusted according to a weighted average of the previous period's inflation (π_t1) and steady-state inflation (π_star).",
                     tex_label="\\zeta_p")
  end
  ...
  if subspec(m) in ["ss23", "ss24", "ss25", "ss26", "ss28", "ss29", "ss43", "ss44"]
      ...
      m <= parameter(:ρ_r2, .8258, (1e-5, 0.999), (1e-5, 0.999), ModelConstructors.SquareRoot(), BetaAlt(0.75, 0.10), fixed=false,
                     description="ρ: The degree of inertia in the monetary policy rule.",
                     tex_label="\\rho_R")
 ...
 #=  # EVERY REGIME!!!!!
   elseif subspec(m) in ["ss51"]
       m <= parameter(:S′′_r2, 6.3325, (-15.,15.), (-15.,15.), ModelConstructors.Untransformed(), Distributions.Normal(4.,1.5),
        ...
        m <= parameter(:σ_c_r2, 1.2312, (1e-5, 10.), (1e-5, 10.), ModelConstructors.Exponential(), Distributions.Normal(1.5, 0.37),
                       fixed=false,
                       tex_label="\\sigma_{c}")

        m <= parameter(:h_r2, 0.7205, (1e-5, 0.999), (1e-5, 0.999), ModelConstructors.SquareRoot(), BetaAlt(0.7, 0.1), fixed=false,
                       description="h: Consumption habit persistence.",
                       tex_label="h")
        ...
        m <= parameter(:ζ_w_r2, 0.7937, (1e-5, 0.999), (1e-5, 0.999), ModelConstructors.SquareRoot(), BetaAlt(0.5, 0.1), fixed=false,
                       description="ζ_w: (1-ζ_w) is the probability with which households can freely choose wages in each period. With probability ζ_w, wages increase at a geometrically weighted average of the steady state rate of wage increases and last period's productivity times last period's inflation.",
                       tex_label="\\zeta_w")
        ...
        m <= parameter(:ν_l_r2, 2.8401, (1e-5, 10.), (1e-5, 10.), ModelConstructors.Exponential(), Distributions.Normal(2.0, 0.75),
                       fixed=false,
        ...
        m <= parameter(:ι_w_r2, 0.4425, (1e-5, 0.999), (1e-5, 0.999), ModelConstructors.SquareRoot(), BetaAlt(0.5, 0.15), fixed=false,
                       description="ι_w: The weight attributed to last period's wage in wage indexation. (1-ι_w) is the weight attributed to steady-state wages.",
                       tex_label="\\iota_w")

        m <= parameter(:ι_p_r2, 0.3291, (1e-5, 0.999), (1e-5, 0.999), ModelConstructors.SquareRoot(), BetaAlt(0.5, 0.15), fixed=false,
                       description="ι_p: The weight attributed to last period's inflation in price indexation. (1-ι_p) is the weight attributed to steady-state inflation.",
                       tex_label="\\iota_p")

        m <= parameter(:ppsi_r2, 0.2648, (1e-5, 0.999), (1e-5, 0.999), ModelConstructors.SquareRoot(), BetaAlt(0.5, 0.15), fixed=false,
                           description="ppsi: Utilization costs.",
                   tex_label="\\psi")

        m <= parameter(:Φ_r2, 1.4672, (1., 10.), (1.00, 10.00), ModelConstructors.Exponential(), Distributions.Normal(1.25, 0.12),
                       fixed=false,
                       description="Φ: Fixed costs.",
                       tex_label="\\Phi")
        ...
        m <= parameter(:ψ2_r2, 0.0893, (-0.5, 0.5), (-0.5, 0.5), ModelConstructors.Untransformed(), Distributions.Normal(0.12, 0.05),
                       fixed=false,
                       description="ψ₂: Weight on output gap in monetary policy rule.",
                       tex_label="\\psi_2")

        m <= parameter(:ψ3_r2, 0.2239, (-0.5, 0.5), (-0.5, 0.5), ModelConstructors.Untransformed(), Distributions.Normal(0.12, 0.05),
                       fixed=false,
                       description="ψ₃: Weight on rate of change of output gap in the monetary policy rule.",
                       tex_label="\\psi_3")

        m <= parameter(:π_star_r2, 0.7000, (1e-5, 10.), (1e-5, 10.), ModelConstructors.Exponential(), GammaAlt(0.62, 0.1), fixed=false,
                       scaling = x -> 1 + x/100,
                       description="π_star: The steady-state rate of inflation.",
                       tex_label="\\pi_*")

        m <= parameter(:β_r2, 0.7420, (1e-5, 10.), (1e-5, 10.), ModelConstructors.Exponential(), GammaAlt(0.25, 0.1), fixed=false,
                       scaling = x -> 1/(1 + x/100),
                       description="β: Discount rate.",
                       tex_label="\\beta ")

        m <= parameter(:Lmean_r2, 1.2918, (-5., 5.), (-5., 5.), ModelConstructors.Untransformed(), Distributions.Normal(0.0, 2.0),
                       fixed=false, description="Lmean: Mean level of hours.",
                       tex_label="\\bar{L}")

        m <= parameter(:γ_r2, 0.3982, (-5.0, 5.0), (-5., 5.), ModelConstructors.Untransformed(), Distributions.Normal(0.4, 0.1),
                       fixed=false, scaling = x -> 1 + x/100,
                       description="γ: The log of the steady-state growth rate of technology.",
                       tex_label="\\gamma")

        m <= parameter(:α_r2, 0.24, (1e-5, 0.999), (1e-5, 0.999), ModelConstructors.SquareRoot(), Distributions.Normal(0.30, 0.05),
                       fixed=false,
                       description="α: Capital elasticity in the intermediate goods sector's Cobb-Douglas production function.",
                       tex_label="\\alpha")
        ...
        # exogenous processes - autocorrelation
        m <= parameter(:ρ_z_r2, 0.9676, (1e-5, 0.999), (1e-5, 0.999), ModelConstructors.SquareRoot(), BetaAlt(0.5, 0.2), fixed=false,
                       description="ρ_z: AR(1) coefficient in the technology process.",
                       tex_label="\\rho_z")

        m <= parameter(:ρ_b_r2, 0.2703, (1e-5, 0.999), (1e-5, 0.999), ModelConstructors.SquareRoot(), BetaAlt(0.5, 0.2), fixed=false,
                       description="ρ_b: AR(1) coefficient in the intertemporal preference shifter process.",
                       tex_label="\\rho_b")
        #added 4 9's to upper bound for the following parameter
        m <= parameter(:ρ_g_r2, 0.9930, (1e-5,0.9999999), (1e-5,0.9999999), ModelConstructors.SquareRoot(), BetaAlt(0.5, 0.2), fixed=false,
                       description="ρ_g: AR(1) coefficient in the government spending process.",
                       tex_label="\\rho_g")

        m <= parameter(:ρ_μ_r2, 0.5724, (1e-5, 0.999), (1e-5, 0.999), ModelConstructors.SquareRoot(), BetaAlt(0.5, 0.2), fixed=false,
                       description="ρ_μ: AR(1) coefficient in capital adjustment cost process.",
                       tex_label="\\rho_{\\mu}")

        m <= parameter(:ρ_rm_r2, 0.3000, (1e-5, 0.999), (1e-5, 0.999), ModelConstructors.SquareRoot(), BetaAlt(0.5, 0.2), fixed=false,
                       description="ρ_rm: AR(1) coefficient in the monetary policy shock process.", # CHECK THIS
                       tex_label="\\rho_{r^m}")

        m <= parameter(:ρ_λ_f_r2, 0.8692, (1e-5, 0.999), (1e-5, 0.999), ModelConstructors.SquareRoot(), BetaAlt(0.5, 0.2), fixed=false,
                       description="ρ_λ_f: AR(1) coefficient in the price mark-up shock process.",
                       tex_label="\\rho_{\\lambda_f}")

        m <= parameter(:ρ_λ_w_r2, 0.9546, (1e-5, 0.9999), (1e-5, 0.999), ModelConstructors.SquareRoot(), BetaAlt(0.5, 0.2), fixed=false,
                       description="ρ_λ_w: AR(1) coefficient in the wage mark-up shock process.", # CHECK THIS
                       tex_label="\\rho_{\\lambda_w}")

        m <= parameter(:η_λ_f_r2, 0.7652, (1e-5, 0.99999), (1e-5, 0.99999), ModelConstructors.SquareRoot(), BetaAlt(0.50, 0.20), fixed=false,
                       description="η_λ_f: Moving average component in the price markup shock.",
                       tex_label="\\eta_{\\lambda_f}")

        m <= parameter(:η_λ_w_r2, 0.8936, (1e-5, 0.999), (1e-5, 0.999), ModelConstructors.SquareRoot(), BetaAlt(0.50, 0.20), fixed=false,
                       description="η_λ_w: Moving average component in the wage markup shock.",
                       tex_label="\\eta_{\\lambda_w}")

        m <= parameter(:η_gz_r2, 0.0500, (1e-5, 0.999), (1e-5, 0.999), ModelConstructors.SquareRoot(), BetaAlt(0.50, 0.20), fixed=false,
                       description="η_gz: Correlate g and z shocks.",
                       tex_label="\\eta_{gz}")
