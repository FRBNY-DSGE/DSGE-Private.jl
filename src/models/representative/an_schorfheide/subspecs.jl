...
function ss4!(m::AnSchorfheide)
    ...
    b = 0.
    m <= parameter_ad(:τ, 1.9937, (0., Inf), (0., b), ModelConstructors.Exponential(), GammaAlt(2., 0.5), fixed=true,
                   description="τ: The inverse of the intemporal elasticity of substitution.",
                   tex_label="\\tau")

    m <= parameter_ad(:κ, 0.7306, (0., 1.), (0., 1.), ModelConstructors.SquareRoot(), Uniform(0,1), fixed=true,
                   description="κ: Composite parameter in New Keynesian Phillips Curve.",
                   tex_label="\\kappa")

    m <= parameter_ad(:ψ_1, 1.1434, (0., Inf), (0., b), ModelConstructors.Exponential(), GammaAlt(1.5, 0.25), fixed=true,
                   description="ψ_1: The weight on inflation in the monetary policy rule.",
                   tex_label="\\psi_1")
    m <= parameter_ad(:ψ_2, 0.4536, (0., Inf), (0., b), ModelConstructors.Exponential(), GammaAlt(0.5, 0.25), fixed=true,
                   description="ψ_2: The weight on the output gap in the monetary policy rule.",
                   tex_label="\\psi_2")

    m <= parameter_ad(:rA, 0.0313, (0., Inf), (0., b), ModelConstructors.Exponential(), GammaAlt(0.5, 0.5), fixed=true,
                   description="rA: β (discount factor) = 1/(1+ rA/400).",
                   tex_label="rA")

    m <= parameter_ad(:π_star, 8.1508, (0., Inf), (0., b), ModelConstructors.Exponential(), GammaAlt(7., 2.), fixed=true,
                   description="π_star: Target inflation rate.",
                   tex_label="\\pi*")

    m <= parameter_ad(:γ_Q, 1.5, (0., Inf), (0., b), ModelConstructors.Exponential(), Normal(0.40, 0.20), fixed=true,
                   description="γ_Q: Steady state growth rate of technology.",
                   tex_label="\\gamma_Q")

    m <= parameter_ad(:ρ_R, 0.3847, (0., 1.), (0., 1.), ModelConstructors.SquareRoot(), Uniform(0,1), fixed=true,
                   description="ρ_R: AR(1) coefficient on interest rate.",
                   tex_label="\\rho_R")

    m <= parameter_ad(:ρ_g, 0.3777, (0., 1.), (0., 1.), ModelConstructors.SquareRoot(), Uniform(0,1), fixed=true,
                   description="ρ_g: AR(1) coefficient on g_t = 1/(1 - ζ_t), where ζ_t is government spending as a fraction of output.",
                   tex_label="\\rho_g")

    m <= parameter_ad(:ρ_z, 0.9579, (0., 1.), (0., 1.), ModelConstructors.SquareRoot(), Uniform(0.,1.), fixed=true,
                   description="ρ_z: AR(1) coefficient on shocks to the technology growth rate.",
                   tex_label="\\rho_z")

    m <= parameter_ad(:σ_R, 0.4900, (1e-5, 10.), (1e-5, 10.), ModelConstructors.SquareRoot(), RootInverseGamma(4, .4), fixed=false,
                   description="σ_R: Standard deviation of shocks to the nominal interest rate.",
                   tex_label="\\sigma_R")

    m <= parameter_ad(:σ_g, 1.4594, (1e-5, 10.), (1e-5, 10.), ModelConstructors.SquareRoot(), RootInverseGamma(4, 1.), fixed=false,
                   description="σ_g: Standard deviation of shocks to the government spending process.",
                   tex_label="\\sigma_g")

    m <= parameter_ad(:σ_z, 0.9247, (1e-5, 10.), (1e-5, 10.), ModelConstructors.SquareRoot(), RootInverseGamma(4, 0.5), fixed=false,
                   description="σ_z: Standard deviation of shocks to the technology growth rate process.",
                   tex_label="\\sigma_z")
    m <= parameter_ad(:e_y, 0., fixed=true,
                   description="e_y: Measurement error on GDP growth.",
                   tex_label="e_y")

    m <= parameter_ad(:e_π, 0., fixed=true,
                   description="e_π: Measurement error on inflation.",
                   tex_label="e_\\pi")

    m <= parameter_ad(:e_R, 0., fixed=true,
                   description="e_R: Measurement error on the interest rate.",
                   tex_label="e_R")
end
