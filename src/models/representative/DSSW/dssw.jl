...
function init_parameters!(m::DSSW)
    m <= parameter(:alp, 0.25, (1e-5, 0.99999), (1e-5, 0.99), ModelConstructors.SquareRoot(), BetaAlt(0.33, 0.1), fixed=false,
    ...
    m <= parameter(:iota_p, 0., (0., 1.), (1e-5, 0.99), ModelConstructors.SquareRoot(), BetaAlt(0.5, 0.28), fixed=true,
    ...
    m <= parameter(:del, 0.025, (1e-5, 0.99999), (1e-5, 0.99), ModelConstructors.SquareRoot(), BetaAlt(0.025, 0.01), fixed=true,
                   description="δ: The capital depreciation rate.",
                   tex_label="\\delta" )

    m <= parameter(:ups, 0., (0., 2.), (1e-5, 0.), ModelConstructors.Exponential(), GammaAlt(1., 0.5), fixed=true, scaling = x -> exp(x/100),
                   description="Ups",
                   tex_label="\\Upsilon")

    m <= parameter(:bigphi, 0., (0., 2.), (1e-5, 0.), ModelConstructors.Exponential(), GammaAlt(0.5, 0.25), fixed=true,
                   description="Φ: Fixed costs.",
                   tex_label="\\Phi")

    m <= parameter(:s2, 4., (0., 20.), (1e-5, 0.), ModelConstructors.Exponential(), GammaAlt(4.0, 1.5), fixed=false,
    ...
    m <= parameter(:a2, 0.2, (0., 0.99999), (1e-5, 0.), ModelConstructors.Exponential(), GammaAlt(0.2, 0.1), fixed=false,
                   description="ppsi: Utilization costs.",
                   tex_label="a''")

    m <= parameter(:nu_l, 2., (1e-5, 10.), (1e-5, 0.), ModelConstructors.Exponential(), GammaAlt(2.0, 0.75), fixed=false,
    ...
    m <= parameter(:law, 0.3, (1e-5, 50.), (1e-5, 0.), ModelConstructors.Exponential(), GammaAlt(0.3, 0.5), fixed=true,
                   description="λ_w: The wage markup, which affects the elasticity of substitution between differentiated labor services.",
                   tex_label="\\lambda_w")

    m <= parameter(:bet, .5, (1e-5, 10.), (1e-5, 0.), ModelConstructors.Exponential(), GammaAlt(2.0, 1.0), fixed=false, scaling = x -> exp(-x/100),#scaling = x -> 1/(1 + x/100),
                   description="β: Discount rate.",
                   tex_label="\\beta")

    m <= parameter(:psi1, 1.7, (1e-5, 10.), (1e-5, 0.), ModelConstructors.Exponential(), GammaAlt(1.5, 0.4), fixed=false,
                   description="ψ₁: Weight on inflation gap in monetary policy rule.",
                   tex_label="\\psi_1")

    m <= parameter(:psi2, 0.125, (1e-5, 10.), (1e-5, 0.), ModelConstructors.Exponential(), GammaAlt(0.2, 0.1), fixed=false,
                   description="ψ₂: Weight on output gap in monetary policy rule.",
                   tex_label="\\psi_2")

    m <= parameter(:rho_r, 0.8, (1e-5, 0.99999), (1e-5, 0.99), ModelConstructors.SquareRoot(), BetaAlt(0.5, 0.2), fixed=false,
                   description="ρ_r",
                   tex_label="\\rho_r")
...
elseif subspec(m) in ["ss1"]
    ...
    m <= parameter(:laf, 0.3, (1e-5, 50.), (1e-5, 0.), ModelConstructors.Exponential(), GammaAlt(0.15, 0.1), fixed=true,
                   tex_label="\\lambda_{f}")

    m <= parameter(:gstar, 0.15, (1e-5, 0.99999), (1e-5, 0.99), ModelConstructors.SquareRoot(), GammaAlt(0.3, 0.1), fixed=false, scaling = x -> 1 + x,
                   tex_label="\\g_{*}")
    #=
    m <= parameter(:Ladj, 5., (1e-5, 10.), (0., 0.), ModelConstructors.Untransformed(), Normal(252.0, 10.0), fixed=false,
...
=#
    ...
    m <= parameter(:rho_z, .2, (0., 0.99999), (1e-5, 0.99), ModelConstructors.SquareRoot(), BetaAlt(0.2, 0.1), fixed=false,
                   ...
                   tex_label="\\rho_z")
    m <= parameter(:rho_phi, 0.4, (1e-5, 0.99999), (1e-5, 0.99), ModelConstructors.SquareRoot(), BetaAlt(0.6, 0.2), fixed=false,
                   description="ρ_ϕ: AR(1) coefficient in the government spending process.",
                   tex_label="\\rho_{\\phi}")

    m <= parameter(:rho_chi, 0.9, (1e-5, 0.99999), (1e-5, 0.99), ModelConstructors.SquareRoot(), BetaAlt(0.6, 0.2), fixed=true,
                   description="ρ_χ: AR(1) coefficient in the government spending process.",
                   tex_label="\\rho_{\\chi}")

    m <= parameter(:rho_laf, 0.9, (1e-5, 0.99999), (1e-5, 0.99), ModelConstructors.SquareRoot(), BetaAlt(0.6, 0.2), fixed=false,
                   description="ρ_λ_f: AR(1) coefficient in the price mark-up shock process.",
                   tex_label="\\rho_{\\lambda_f}")

    m <= parameter(:rho_mu, 0.5, (1e-5, 0.99999), (1e-5, 0.99), ModelConstructors.SquareRoot(), BetaAlt(0.8, 0.05), fixed=false,
                   description="ρ_μ: AR(1) coefficient in capital adjustment cost process.",
                   tex_label="\\rho_{\\mu}")

    m <= parameter(:rho_b, 0.8, (1e-5, 0.99999), (1e-5, 0.99), ModelConstructors.SquareRoot(), BetaAlt(0.6, 0.2), fixed=false,
                   description="ρ_b: AR(1) coefficient in the intertemporal preference shifter process.",
                   tex_label="\\rho_b")

    m <= parameter(:rho_g, 0.9, (1e-5, 0.99999), (1e-5, 0.99), ModelConstructors.SquareRoot(), BetaAlt(0.8, 0.05), fixed=false,
                   description="ρ_g: AR(1) coefficient in the government spending process.",
                   tex_label="\\rho_g")

    m <= parameter(:sig_z, 0.4, (1e-7, 100.), (1e-5, 0.), ModelConstructors.Exponential(), RootInverseGamma(2., 0.75), fixed=false,
                   description="σ_z: The standard deviation of the process describing the stationary component of productivity.",
                   tex_label="\\sigma_{z}")

    m <= parameter(:sig_phi, 1., (1e-7, 100.), (1e-5, 0.), ModelConstructors.Exponential(), RootInverseGamma(2., 4.), fixed=false,
                   description="σ_z: The standard deviation of the process describing the stationary component of productivity.",
                   tex_label="\\sigma_{\\phi}")

    m <= parameter(:sig_chi, 0., (1e-7, 100.), (1e-5, 0.), ModelConstructors.Exponential(), RootInverseGamma(2., 0.75), fixed=true,
    ...
    m <= parameter(:sig_laf, 1., (1e-7, 10000.), (1e-5, 0.), ModelConstructors.Exponential(), RootInverseGamma(2., 0.75), fixed=false,
    ...
    m <= parameter(:sig_mu, 1., (1e-7, 100.), (1e-5, 0.), ModelConstructors.Exponential(), RootInverseGamma(2., 0.75), fixed=false,
                   description="σ_μ: The standard deviation of the exogenous marginal efficiency of investment shock process.",
                   tex_label="\\sigma_{\\mu}")

    m <= parameter(:sig_b, 0.2, (1e-7, 100.), (1e-5, 0.), ModelConstructors.Exponential(), RootInverseGamma(2., 0.75), fixed=false,
                   description="σ_b: The standard deviation of the intertemporal preference shifter process.",
                   tex_label="\\sigma_{b}")

    m <= parameter(:sig_g, 0.3, (1e-7, 100.), (1e-5, 0.), ModelConstructors.Exponential(), RootInverseGamma(2., 0.75), fixed=false,
                   description="σ_g: The standard deviation of the government spending process.",
                   tex_label="\\sigma_{g}")

    m <= parameter(:sig_r, 0.1, (1e-7, 100.), (1e-5, 0.), ModelConstructors.Exponential(), RootInverseGamma(2., 0.20), fixed=false,
                   tex_label="\\sigma_{r}")
...
function model_settings!(m::DSSW)
    ...
    m <= Setting(:saveroot, "/data/dsge_data_dir/dsgejl/ibrahima/vecm_2007/dsgevecm/")
end
