...
function init_parameters!(m::DSSW)
    ...
    m <= parameter(:iota_p, 0., (0., 1.), (0., 1.), ModelConstructors.SquareRoor(), BetaAlt(0., 0.), fixed=true,
    ...
    m <= parameter(:del, 0.025,(1e-5, 0.99999), (1e-5, 0.99999), ModelConstructors.SquareRoot(), BetaAlt(0.025, 0.01), fixed=true,
                   description="δ: The capital depreciation rate.",
                   tex_label="\\delta" )

    m <= parameter(:ups, 0., (0., 2.), (0., 2.), ModelConstructors.Exponential(), GammaAlt(0., 0.), fixed=true, scaling = x -> exp(x/100),
                   description="Ups",
                   tex_label="\\Upsilon")

    m <= parameter(:bigphi, 0., (0., 2.), (0., 2.), ModelConstructors.Exponential(), GammaAlt(0., 0.), fixed=true,
                   description="Φ: Fixed costs.",
                   tex_label="\\Phi")

    m <= parameter(:s2, 4., (0., 20.), (0., 20.), ModelConstructors.Exponential(), GammaAlt(4.0, 1.5), fixed=false,
    ...
    m <= parameter(:a2, 0.2, (0., 0.99999), (0., 0.99999), ModelConstructors.Exponential(), GammaAlt(0.2, 0.1), fixed=false,
    ...
    m <= parameter(:nu_m, 2., (1e-5, 100.), (1e-5, 100.), ModelConstructors.Exponential(), GammaAlt(2.0, 0.), fixed=true,                                   description="ν_l: The coefficient of relative risk aversion on the labor term of households' utility function.", tex_label="\\nu_m")
    ...
    m <= parameter(:iota_w, 0., (0., 1.), (0., 1.), ModelConstructors.SquareRoot(), BetaAlt(0., 0.), fixed=true,
    ...
    m <= parameter(:law, 0.3,(1e-5, 50.), (1e-5, 50.), ModelConstructors.Exponential(), GammaAlt(0.3, 0.5), fixed=true,
    ...
    m <= parameter(:psi1, 1.7, (1e-5, 10.), (1e-5, 10.), ModelConstructors.Exponential(), GammaAlt(1.5, 0.4), fixed=false,
                   description="ψ₁: Weight on inflation gap in monetary policy rule.",
                   tex_label="\\psi_1")

    m <= parameter(:psi2, 0.125, (1e-5, 10.), (1e-5, 10.), ModelConstructors.Exponential(), GammaAlt(0.2, 0.1), fixed=false,
                   description="ψ₂: Weight on output gap in monetary policy rule.",
                   tex_label="\\psi_2")

    m <= parameter(:rho_r, 0.8, (1e-5, 0.99999), (1e-5, 0.99999), ModelConstructors.SquareRoot(), BetaAlt(0.5, 0.2), fixed=false,
                   description="ρ_r",
                   tex_label="\\rho_r")
    ...
    m <= parameter(:wadj, 5.5, (0., 10.), (0., 10.), ModelConstructors.Exponential(), Normal(0.0, 5.0), fixed=false,
                   tex_label="\\w_{adj}")

    m <= parameter(:chi, 0.1, (1e-6, 10.), (1e-6, 10.), ModelConstructors.Exponential(), GammaAlt(0.1, 0.), fixed=true,
                   tex_label="\\chi")

    m <= parameter(:laf, 0.3, (1e-5, 50.), (1e-5, 50.), ModelConstructors.Exponential(), GammaAlt(0.3, 0.), fixed=true,
                   tex_label="\\lambda_{f}")

    m <= parameter(:gstar, 0.15, (1e-5, 0.99999), (1e-5, 0.99999), ModelConstructors.SquareRoot(), GammaAlt(0.3, 0.1), fixed=false, scaling = x -> 1 + x,
                   tex_label="\\g_{*}")
    ...
    m <= parameter(:rho_chi, 0.9, (1e-5, 0.99999), (1e-5, 0.99999), ModelConstructors.SquareRoot(), BetaAlt(0.9, 0.), fixed=true,
    ...
    m <= parameter(:sig_chi, 0.01, (1e-7, 100.), (1e-7, 100.), ModelConstructors.Exponential(), RootInverseGamma(0., 0.75), fixed=true,
