"""
```
AnSchorfheideSP{T} <: AbstractRepModel{T}
```

The `AnSchorfheideSP` type defines the structure of the simple New Keynesian DSGE
...
"""
mutable struct AnSchorfheideSP{T} <: AbstractRepModel{T}
...
description(m::AnSchorfheideSP) = "Serra's copy of Julia implementation of model defined in 'Bayesian Estimation of DSGE Models' by Sungbae An and Frank Schorfheide: AnSchorfheideSP, $(m.subspec)"

"""
`init_model_indices!(m::AnSchorfheideSP)`

Arguments:
`m:: AnSchorfheideSP`: a model object

Description:
Initializes indices for all of `m`'s states, shocks, and equilibrium conditions.
"""
function init_model_indices!(m::AnSchorfheideSP)
    # Endogenous states
...
function AnSchorfheideSP(subspec::String="ss0";
...
"""
```
init_parameters!(m::AnSchorfheideSP)
```
...
those).
"""
function init_parameters!(m::AnSchorfheideSP)
    # Initialize parameters
...
function shock_groupings(m::AnSchorfheideSP)
