...
"""
mutable struct PLT{T} <: AbstractRepModel{T}     # Only thing you should modify here is the name `AnSchorfheide`
...
description(m::PLT) = "Julia implementation of model defined in 'Bayesian Estimation of DSGE Models' by Sungbae An and Frank Schorfheide: AnSchorfheide, $(m.subspec)"
...
function PLT(subspec::String="ss0"; # do not edit inputs
...
function shock_groupings(m::AnSchorfheide)
    u = ShockGroup("u", [:u_sh], RGB(0.70, 0.13, 0.13)) # firebrick
    r = ShockGroup("r", [:r_sh], RGB(1.0, 0.55, 0.0)) # darkorange
