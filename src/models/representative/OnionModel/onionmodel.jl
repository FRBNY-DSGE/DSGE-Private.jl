...
function shock_groupings(m::OnionModel)
    ...
    pmu_trend = ShockGroup("mkp_trend", [Symbol("μ_trend_$(i)_sh") for i in 1:get_setting(m, :n_sectors) for i in 1:get_setting(m, :n_sectors)], RGB(0.0, 0.8, 0.0))
    pmu_iid = ShockGroup("mkp_iid", [Symbol("μ_iid_$(i)_sh") for i in 1:get_setting(m, :n_sectors) for i in 1:get_setting(m, :n_sectors)], RGB(0.5, 0.5, 0.0))
    tax = ShockGroup("tax", [:τ_sh], RGB(0.29, 0.0, 0.51))

    return [pmu_trend, pmu_iid, tax]
end
