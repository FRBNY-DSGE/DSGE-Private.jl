...
=#
    ...
    B[eq[Symbol("eq_pc_1")]:eq[Symbol("eq_pc_$n")], endo[:τ]]  = - sec_shock
    A[eq[Symbol("eq_pc_1")]:eq[Symbol("eq_pc_$n")], endo[:lw]] =  m[:labshare].value
    B[eq[Symbol("eq_pc_1")]:eq[Symbol("eq_pc_$n")], endo[:a_t]] =  -m[:labshare].value
    ...
    B[eq[Symbol("eq_pc_1")]:eq[Symbol("eq_pc_$n")], endo[Symbol("mkup_1")]:endo[Symbol("mkup_$n")]] = - diagm(m[:invkap].value) # eye(n) #Addl term for markup shocks

    Π[eq[Symbol("eq_pc_1")]:eq[Symbol("eq_pc_$n")], exp_sh[Symbol("Eπ_1_sh")]:exp_sh[Symbol("Eπ_$(n)_sh")]] =  eye(n) #Really not convinced by this term being here...
...
=#
    ...
    A[eq[:eq_wpc],endo[:lw]]  = - 1.
    B[eq[:eq_wpc], endo[:lc]] = m[:h] * exp(-m[:γ])
    B[eq[:eq_wpc], endo[:μw]] = m[:invkapw] #1.
    Π[eq[:eq_wpc],exp_sh[:Eπw_sh]] = 1.
...
#Not convinced we need the following anymore..
...
#Further define the stochastic trend:
