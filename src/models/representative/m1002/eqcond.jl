...
function eqcond(m::Model1002)
    ...
    if n_z_anticipated_shocks(m) > 0
        ...
        Γ1[eq[:eq_z], endo[:z_tl1]]   = 1.
        Γ0[eq[:eq_zl1], endo[:z_tl1]] = 1.
        Ψ[eq[:eq_zl1], exo[:z_shl1]]  = 1.

        if n_z_anticipated_shocks(m) > 1
            for i = 2:n_z_anticipated_shocks(m)
                Γ1[eq[Symbol("eq_zl$(i-1)")], endo[Symbol("z_tl$i")]] = 1.
                Γ0[eq[Symbol("eq_zl$i")], endo[Symbol("z_tl$i")]]     = 1.
                Ψ[eq[Symbol("eq_zl$i")], exo[Symbol("z_shl$i")]]      = 1.
            end
        end
    end
