...
alternative_policy(m::AbstractDSGEModel) = haskey(get_settings(m), :regime_eqcond_info) && !isempty(get_setting(m, :regime_eqcond_info)) && # no check for n_regimes b/c if it is not there, then
    ...
    ((haskey(get_settings(m), :regime_eqcond_info) && !isempty(get_setting(m, :regime_eqcond_info))) ? get_setting(m, :regime_eqcond_info)[sort(collect(keys(get_setting(m, :regime_eqcond_info))))[end]].alternative_policy :
