...
function compute_system(m::GHLS; apply_altpolicy = false,
    ...
    @inline Φ(s_t1::Vector{Float64}, ϵ_t::Vector{Float64}) = append!(decr(m.approx,s_t1, ϵ_t, m.parameters, m.keys, m[:labss], α_star, m.exogenous_shocks, m.endogenous_states), [s_t1[m.endogenous_states[:y_t]], s_t1[m.endogenous_states[:c_t]], s_t1[m.endogenous_states[:i_t]]])
    Ψ = measurement(m)
