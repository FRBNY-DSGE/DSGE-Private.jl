...
#=
    draws   = transpose(readdlm(path * "001parasim.txt"))
    if parallel
        draws, loglh, logpost = @sync @parallel (vector_reduce) for i in 1:n_parts
            draw = vec(draws[i]) #vec(rand(m.parameters, 1))
            draw_loglh = 0.
            draw_logpost = 0.
            success = false
            while !success
    ...
    draws_orig   = transpose(readdlm(path * "001parasim.txt"))
    loglh_orig   = vec(readdlm(path * "001liksim.txt"))
    logpost_orig = vec(readdlm(path * "001postsim.txt"))

    @test_matrix_approx_eq draws_orig draws
    @test_matrix_approx_eq loglh_orig loglh
    @test_matrix_approx_eq logpost_orig logpost
=#
    draws   = transpose(readdlm(path * "001parasim.txt"))
    loglh   = vec(readdlm(path * "001liksim.txt"))
    logpost = vec(readdlm(path * "001postsim.txt")) - loglh
    # redraw at 11371
