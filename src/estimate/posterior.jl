...
function likelihood(m::AbstractDSGEModel, data::AbstractMatrix;
    ...
    try
        #=if add_zlb_duration[1] && !isa(m, PoolModel) && use_chand_recursion == false ## Not set up for PoolModel or use_chand_recursion = true
            ...
            if isa(m, PoolModel)
                println("pool model likelihood")
                return ψ_l * sum(filter_likelihood(m, data; tol = tol,
                                                   tuning = get_setting(m, :tuning))) + ψ_p * penalty
            elseif isa(m, DSSW)
                println("dssw likelihood")
                return dsge_coint_likelihood(m, data)
            elseif use_chand_recursion==false
                println("chandrecursion_likelihood")
                lh = filter_likelihood(m, data, system; add_zlb_duration = add_zlb_duration,
                                                   include_presample = false, tol = tol)

                #TODO: fix everything to accept return both, previously only was return 1st item
                return ψ_l * sum(lh) + ψ_p * penalty, lh
            else
                # TODO: extend chand_recursion for a regime-switching system
                println("chandrecursionregime_likelihood")
