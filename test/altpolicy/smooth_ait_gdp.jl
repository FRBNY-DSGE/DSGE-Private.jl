...
Γ0_alt, Γ1_alt, C_alt, Ψ_alt, Π_alt = DSGE.smooth_ait_gdp_eqcond(m)

endo = m.endogenous_states
eq   = m.equilibrium_conditions

# Default coefficients (ss0, no overrides). Note: φ defaults to 0.25 here (vs
# 11.13 in smooth_ait_gdp_alt) and the gap loading uses 1/(1-ρ), not (1-ρ).
...
@testset "smooth_ait_gdp AltPolicy object is wired up correctly" begin
    pol = DSGE.smooth_ait_gdp()
    @test pol isa DSGE.AltPolicy
    @test pol.key           == :smooth_ait_gdp
    @test pol.eqcond        === DSGE.smooth_ait_gdp_eqcond
    @test pol.solve         === DSGE.smooth_ait_gdp_solve
    @test pol.forecast_init === DSGE.smooth_ait_gdp_forecast_init
end

@testset "smooth_ait_gdp adds the pgap and ygap states and conditions" begin
...
@testset "smooth_ait_gdp price-gap law of motion" begin
...
@testset "smooth_ait_gdp GDP-gap law of motion" begin
...
@testset "smooth_ait_gdp smoothed MP rule" begin
    @test Γ0_alt[eq[:eq_mp], endo[:R_t]]    == 1.
    @test Γ1_alt[eq[:eq_mp], endo[:R_t]]    == ρ_smooth
    @test Γ0_alt[eq[:eq_mp], endo[:pgap_t]] == -φ_π * (1. / (1. - ρ_pgap)) * (1. - ρ_smooth)
    @test Γ0_alt[eq[:eq_mp], endo[:ygap_t]] == -φ_y * (1. / (1. - ρ_ygap)) * (1. - ρ_smooth)
    @test C_alt[eq[:eq_mp]]                 == 0.
    # MP shock is fully removed (no rm_t term)
    @test all(Ψ_alt[eq[:eq_mp], :] .== 0.)
end

@testset "smooth_ait_gdp leaves the non-policy equilibrium conditions unchanged" begin
...
@testset "ait_Thalf / gdp_Thalf settings feed through to ρ_pgap / ρ_ygap" begin
    mt = SmetsWouters(custom_settings = [Setting(:ait_Thalf, 20.),
                                         Setting(:gdp_Thalf, 5.)])
    Γ0_t, Γ1_t, ~ = DSGE.smooth_ait_gdp_eqcond(mt)
...
@testset "smooth_ait_gdp_solve returns consistently sized, finite transition matrices" begin
    TTT, RRR, CCC = DSGE.smooth_ait_gdp_solve(m)
...
@testset "smooth_ait_gdp_forecast_init seeds pgap / ygap" begin
    ...
    new_shocks, fs = DSGE.smooth_ait_gdp_forecast_init(m, shocks, copy(final_state))
...
if run_benchmarks
    ...
    b_construct = @benchmark DSGE.smooth_ait_gdp()
    b_eqcond    = @benchmark DSGE.smooth_ait_gdp_eqcond($mb)
    b_solve     = @benchmark DSGE.smooth_ait_gdp_solve($mb)

    shocks      = zeros(n_shocks_exogenous(mb), 1)
    final_state = collect(1.0:n_states_augmented(mb))
    b_finit     = @benchmark DSGE.smooth_ait_gdp_forecast_init($mb, $shocks, copy($final_state))

    println("\n===== smooth_ait_gdp benchmark results =====")
    for (name, b) in [("smooth_ait_gdp",               b_construct),
                      ("smooth_ait_gdp_eqcond",        b_eqcond),
                      ("smooth_ait_gdp_solve",         b_solve),
                      ("smooth_ait_gdp_forecast_init", b_finit)]
        println(rpad(name, 32), " time: ", rpad(BenchmarkTools.prettytime(median(b).time), 12),
