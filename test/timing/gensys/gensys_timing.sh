...
for i in $ALL_NODES;
do
  echo -n "node: $i, seconds: " >> gensys_results_jl.txt
  echo -n "node: $i, seconds:" >> gensys_results_mat.txt
  if ((SUBMIT_FORWARD));
  then 
    ../matlab14a-custom-node -n $i -r gensys_speed | grep -E -o '\s[0-9]+\.[0-9]+' >> gensys_results_mat.txt
    ../julia-0.3.9-custom-node -n $i gensys_speed.jl >> gensys_results_jl.txt
  else
    ../julia-0.3.9-custom-node -n $i gensys_speed.jl >> gensys_results_jl.txt
    ../matlab14a-custom-node -n $i -r gensys_speed | grep -E -o '\s[0-9]+\.[0-9]+' >> gensys_results_mat.txt
  fi
