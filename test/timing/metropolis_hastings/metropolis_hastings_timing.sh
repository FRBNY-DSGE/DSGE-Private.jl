...
do
  if ((SUBMIT_FORWARD));
  then 
    echo "../matlab14a-custom-node -n $i -r \"metropolis_hastings_speed $i\" >> metropolis_hastings_results_mat.txt" | batch
    echo "../julia-0.3.9-custom-node -n $i metropolis_hastings_speed.jl $i >> metropolis_hastings_results_jl.txt" | batch
  else
    echo "../julia-0.3.9-custom-node -n $i metropolis_hastings_speed.jl $i >> metropolis_hastings_results_jl.txt" | batch
    echo "../matlab14a-custom-node -n $i -r \"metropolis_hastings_speed $i\" >> metropolis_hastings_results_mat.txt" | batch
  fi
