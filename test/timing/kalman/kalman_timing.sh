...
do
  if ((SUBMIT_FORWARD));
  then 
    echo "../matlab14a-custom-node -n $i -r \"kalman_speed $i\" >> kalman_results_mat.txt" |  batch
    echo "../julia-0.3.9-custom-node -n $i kalman_speed.jl $i >> kalman_results_jl.txt" | batch
  else
    echo "../julia-0.3.9-custom-node -n $i kalman_speed.jl $i >> kalman_results_jl.txt" | batch
    echo "../matlab14a-custom-node -n $i -r \"kalman_speed $i\" >> kalman_results_mat.txt" | batch
  fi
