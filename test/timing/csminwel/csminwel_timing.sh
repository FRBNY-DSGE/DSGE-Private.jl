...
do
  if ((SUBMIT_FORWARD));
  then 
    echo "../matlab14a-custom-node -n $i -r \"csminwel_speed $i\" >> csminwel_results_mat.txt" | batch
    echo "../julia-0.3.9-custom-node -n $i csminwel_speed.jl $i >> csminwel_results_jl.txt" | batch
  else
    echo "../julia-0.3.9-custom-node -n $i csminwel_speed.jl $i >> csminwel_results_jl.txt" | batch
    echo "../matlab14a-custom-node -n $i -r \"csminwel_speed $i\" >> csminwel_results_mat.txt" | batch
  fi
