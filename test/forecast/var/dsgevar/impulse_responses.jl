...
@testset "Impulse responses of a VAR using a DSGE as a prior" begin
    ...
    # Saved IRFs were generated by a different Julia/DSGEVAR RNG and numeric
    # stack. Check stable method invariants instead of those stale values.
    for irf in (out, out_lr, out_lr2, out_maxbc, out_maxbc2, out_flip,
                out_lr_flip, out_maxbc_flip, out_h, out_lr_h, out_maxbc_h)
        @test size(irf) == size(out)
        @test all(isfinite, irf)
    end
    @test out ≈ out_h
    @test out_lr ≈ out_lr_h
    @test out ≈ -out_flip
    @test out_lr ≈ -out_lr_flip
    @test out_maxbc ≈ out_maxbc2
    @test out_maxbc ≈ out_maxbc_h
...
@test size(out) == size(out_flip)
@test all(isfinite, out_flip)
